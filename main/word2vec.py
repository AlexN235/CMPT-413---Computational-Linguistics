...
class Word2vec:
    ...
    def run(self):
        ...
        for e in range(self.epochs):
            ...
            for review in range(len(self.corpus)):
                context, label = get_context_word(self.vocab[train_s:train_e],
                                                  self.rating[train_s:train_e],
                                                  self.vocab_size,
                                                  self.window)
    ...
    def predict(self, x, W1, W2):
        ...
        h = np.mean([np.matmul(self.W1.T, xx) for xx in x], axis=0)
...
def get_predictions(reviews_list, cbow_model, W1, W2):
    ...
    for review in reviews_list:
        one_hot_review = vectorize_words(review,
                                         cbow_model.vocab_size,
                                         cbow_model.tokenizer)
        x = one_hot_review.reshape(-1, cbow_model.vocab_size)
        rating = cbow_model.predict(x, W1, W2)
            
        predictions.append(rating.tolist())
            
    return predictions

## cbow_model
""" Used to save/load weights after training """           
